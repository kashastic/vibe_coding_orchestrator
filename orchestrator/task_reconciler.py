...
# Path components that indicate a source subdirectory, not an app-level folder.
# When a subsequent repo path starts with one of these, it is treated as relative
# to the current app root rather than resetting the root.
_SOURCE_ROOTS = {"src", "lib", "test", "tests", "public", "dist", "build", "static", "assets"}
...
def _expand_repo_paths(repo_path: str) -> list[str]:
    ...
    for part in raw_parts:
        ...
        if len(path_obj.parts) >= 2:
            first = path_obj.parts[0]
            if current_root is not None and first in _SOURCE_ROOTS:
                # Looks like a source-relative path missing its app prefix — prepend it.
                normalized_parts.append(str(Path(current_root) / cleaned))
            else:
                current_root = first
