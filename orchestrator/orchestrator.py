        ...
        while True:
            ...
            try:
                ...
                if selection.task is None:
                    ...
                    if blocked_tasks:
                        notifier.try_send("Codex Blocked", "Tasks remain, but all available work is blocked")
                        ...
                        notifier.try_send("Codex", "All tasks completed")
                ...
                notifier.try_send("Codex Task Started", f"{task.display_name} started")
                ...
                if result.succeeded:
                    ...
                    notifier.try_send("Codex Task Finished", f"{task.task_id} completed successfully")
                    run_logger.append_run(
                        run_logger.build_record(
                            task_id=task.task_id,
                            task_title=task.title,
            ...
            except (NotionAPIError, OSError, ValueError) as exc:
                if run_logger is not None:
                    run_logger.append_run(
                        run_logger.build_record(
                            task_id="SYSTEM",
    ...
    *,
    ...
    for action in plan_reconciliation(all_tasks, repo_root):
        ...
        if action.target_status == "Failed":
            notifier.try_send("Codex Task Failed", f"{action.task.task_id} reconciled to Failed")
...
def _record_blocked_task(
    ...
    *,
    ...
    notifier.try_send("Codex Task Blocked", f"{task_id} blocked: {reason}")
    run_logger.append_run(
        run_logger.build_record(
            task_id=task_id,
